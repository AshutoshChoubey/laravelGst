...
<section class="content" style="margin-left: 20px;margin-right: 20px;">
   {{ Form::open(['url' => '/marketing/add', 'files' => true]) }} 
           {{ csrf_field() }}
...
<div class="card">
           ...
           @foreach($customFields['basic'] as $key => $value)
            @if($value['type'] == 'separator_start')
             ...
             @elseif($value['type'] != 'separator_start' && $value['type'] != 'separator_end')
                                <div class="col-sm-4 {{ isset($value['optColDiv']) ? $value['optColDiv']: '' }}">
                                <div class="form-group {{ $errors->has($key) ? 'has-danger' : ''}}">
                                      ...
                                      $error=$errors->has($key) ? 'is-invalid' : '';
                                       @endphp
                                    @if($value['type'] == 'text')
                                    ...
                                    @elseif($value['type'] == 'textarea')
                                     {{ Form::textarea($key, isset($$key) ? $$key :'', ['class' => 'form-control '.$class.''.$error.' ', 'id' => isset($value['id']) ? $value['id']: '', 'style' => isset($value['style']) ? $value['style']: '', 'placeholder' => $value['label']]) }}
                                     @elseif($value['type'] == 'number')
                                     {{ Form::number($key, isset($$key) ? $$key :'', ['class' => 'form-control '.$class.''.$error.' ', 'id' => isset($value['id']) ? $value['id']: '', 'style' => isset($value['style']) ? $value['style']: '', 'placeholder' => $value['label']]) }}
  ...
  <script type="text/javascript">
    @php if(isset($data['field_disable']) && $data['field_disable'] === true){ @endphp
          ...
          $('#resetBtn').hide();
        @php } @endphp
$(document).on("change keyup keydown","[name='price']",function()
{
 var quantity= $("[name='quantity'").val();
 $("[name='total_price'").val(quantity*$(this).val());
});
  </script>
    <!-- /.content -->
@endsection